...
const fetchFeatures = async () => {
	...
	params.value.app_id = appId.value
	const { load, data, hasLastPage } = getFeatures(params.value);
	await load();
	let featureData = features.value.concat(data.value);
	features.value = featureData.filter(
		(obj, index, self) =>
			index ===
...
const clearFilter = () => {
	emit('clearFeature', selectedFeature.value);
};
...
onMounted(() => {
	...
	emitter.on('ADD_FEATURE', (data) => {
		let ifExistFeature = features.value.find(feature => feature.id == data.id)
		if(!ifExistFeature) {

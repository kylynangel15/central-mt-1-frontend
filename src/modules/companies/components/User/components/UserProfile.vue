	...
	<article v-if="!loadingUser && user && user.id" class="bg-white h-full">
		...
		<div class="mx-auto mt-6 max-w-5xl px-4 sm:px-6 lg:px-8">
			<dl class="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
				...
				<div class="sm:col-span-2">
					<dt class="text-sm font-medium text-gray-500">User Tags</dt>
					<dd class="mt-1 text-sm text-gray-900" v-if="user.tags.length > 0">
						<div class="flex-wrap w-full mt-2">
							<span
								class="mr-2 bg-untitled-gray-400 text-xs text-white p-2 rounded-full"
					...
					<dd class="mt-1 text-sm text-gray-900" v-else>--</dd>
				</div>
			</dl>
		</div>
	</article>
...
import { getUser } from "@/modules/companies/composables/user/get_user";
